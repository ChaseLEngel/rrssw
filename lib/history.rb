...
# Functions for querying, inserting, and deleting records in a SQLite3 database.
class History
  attr_reader :file
  ...
  def initialize(filename)
    ...
    if File.exist? @file
      ...
      RRSSWLogger.debug "Found database:#{@file}"
    else
      create @file
      RRSSWLogger.debug "Created database:#{@file}"
    end
    create_functions
  end
